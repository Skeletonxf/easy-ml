...
mod tensors {
    ...
    #[test]
    fn test_double_ended_shape_iterator_exact_size() {
        let mut iterator = DoubleEndedShapeIterator::from([("x", 3), ("y", 2)]);
        assert_eq!(iterator.size_hint(), (6, Some(6)));
        ...
        let b = iterator.next_back();
        assert_eq!(b, Some([2, 1]));
        assert_eq!(iterator.size_hint(), (4, Some(4)));

        let c = iterator.next();
        assert_eq!(c, Some([0, 1]));
        assert_eq!(iterator.size_hint(), (3, Some(3)));
