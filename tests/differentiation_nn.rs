...
mod neural_net_tests {
    ...
    fn mean_squared_loss<T: Numeric + Copy>(
       inputs: &Vec<Matrix<T>>, w1: &Matrix<T>, w2: &Matrix<T>, labels: &Vec<T>
    ) -> T
    where for<'a> &'a T: NumericRef<T> {
    ...
    fn step_gradient(
        ...
        w1: &mut Matrix<Record<f32>>, w2: &mut Matrix<Record<f32>>,
    ...
    fn test_gradient_descent() {
        ...
        for _ in 0..epochs {
            losses.push(step_gradient(&inputs, &mut w1, &mut w2, &labels, learning_rate, &list))
        }
