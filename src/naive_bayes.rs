...
fn generate_heights(samples: usize, random_numbers: &mut EndlessRandomGenerator) -> Vec<f64> {
    // the average height shall be 1.5 meters
    let heights_distribution = Gaussian::new(1.5, 0.25);
...
fn generate_tail_length(samples: usize, random_numbers: &mut EndlessRandomGenerator) -> Vec<f64> {
    ...
    let tails_distribution = Gaussian::new(1.25, 0.5);
...
fn generate_metabolic_rate(samples: usize, random_numbers: &mut EndlessRandomGenerator) -> Vec<f64> {
    // the average rate shall be 100 heart beats per minute
    let metabolic_rate_distribution = Gaussian::new(100.0, 5.0);
...
fn generate_spiked_tail(samples: usize, random_numbers: &mut EndlessRandomGenerator) -> Vec<f64> {
...
// in the subset because our data is high dimensional which will allow k-means to find 3 cluster
// centres which we will then use to assign all the data to classes.
